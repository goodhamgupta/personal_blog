...
1. Cooperatively loads the matching `T×T` **A-tile** _and_ **B-tile** from global DRAM to **shared memory** (two coalesced 2-D memcpy's).
2. Performs `T` fused-multiply-add sweeps of that data, each thread keeping its running sum in a **register**.
3. Barriers, slides the tile window by `T` along the inner-`k` dimension, and repeats until the dot-product is complete.
4. Finally writes the `T×T` block of `C` back to DRAM.
...
### Updated roofline model

![Updated roofline with tiled GEMM reaching the compute ceiling](mojo_gpu_puzzles/p14_roofline_tiled.png)
