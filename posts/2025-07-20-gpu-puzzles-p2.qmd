...
Beyond coalescing, this arrangement also avoids **shared memory bank conflicts**. Shared memory has 32 banks; when multiple threads access different addresses in the same bank simultaneously, accesses serialize. Our layout ensures `b_shared[k, threadIdx.x]` accesses different banks (or broadcasts `a_shared[threadIdx.y, k]` to all threads), keeping all 32 memory ports busy.

#### Choosing the Right Tile Size

The TPB=32 choice isn't arbitrary—it's a carefully balanced trade-off:

> **Bigger tiles** (64×64): More data reuse, but higher register pressure and lower occupancy  
> **Smaller tiles** (16×16): Better occupancy, fewer bank conflicts, but less data reuse  
> **Sweet spot** (32×32): Fits in 48KB shared memory while maintaining ≥2 blocks per SM

For different workloads, you might need to tune this. Use `nvcc --ptxas-options=-v` to check register usage and Nsight's occupancy calculator to verify you're not leaving performance on the table. More on this in a future post!

#### Beyond Basic Tiling

This implementation scratches the surface of production GEMM optimization. Advanced techniques include:

- **Double buffering**: Overlap the next tile's memory transfer with current tile's computation using `cp.async` instructions
- **Tensor cores**: Modern GPUs can perform 16×16×16 matrix fragments in one instruction via WMMA intrinsics
- **K-splitting**: For huge matrices, split the reduction across multiple kernel launches to avoid register spills

**TLDR**: Barrier, compute, slide to the next tile, repeat. But there's always another level of optimization waiting. 
