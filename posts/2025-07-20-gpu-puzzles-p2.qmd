...
**Larger tiles** (~ 64 × 64): maximise data reuse but spike register/shared-mem usage and can tank occupancy.  
**Smaller tiles** (~ 16 × 16): keep resource pressure low and occupancy high, yet give up arithmetic intensity.  
**Mid-sized tiles** (~ 32 × 32): often the practical "sweet spot", fitting inside a 48 KB shared-mem budget while still allowing two or more blocks per SM.

Exact numbers vary with GPU, kernel, and precision—profile and tune instead of hard-coding a magic constant.

> Full disclosure: I’m still learning the dark art of GPU perf tuning, so I’ll save the details for a future write-up once I’ve had more time to experiment.

#### Beyond Basic Tiling

As mentioned earlier, this implementation scratches the surface of production GEMM optimization. Advanced techniques include:
