...
As the walkthrough video[@nhrfau_roofline_model] shows, the first step in analysing a kernel is to abstract the messy hardware–software dance into a lean, tractable model. 

### Hardware Model Assumptions

Classic roofline assumes ideal hardware and perfect overlap:

![Source: NHR at FAU[@nhrfau_roofline_model]](mojo_gpu_puzzles/p14_hardware.png){fig-align="center" width=40%}

The roofline reduces the entire processor to just two idealised resources:

- **Compute engine** — can execute at most $P_{peak}$ operations per second (FLOP/s, integer ops/s, etc.).  
- **Memory datapath** — can supply/return data at most $b_s$ bytes per second.

In this cartoon the whole GPU collapses to just two levers, $P_{peak} and $b_s$. That only works if three optimistic assumptions hold:
...
We turn the kernel into a single steady-state loop that issues N flops and moves V bytes per trip:

• Every trip around the loop streams V bytes to/from memory.  
• Every trip performs N floating-point operations.  

That single ratio I = N / V (flops per byte) is all that survives of the algorithm; prologue/epilogue work, control flow, synchronisations, and small-constant terms are deliberately swept aside.

For that caricature to be predictive we quietly assume:
...
### Naïve Roofline — the "ideal-case" limit

With the six rosy assumptions (H1-H3, S1-S3) in place, all of hardware × software collapses to one scalar knob—the operational intensity I = N/V [flop/byte]—and the attainable performance becomes
...
At the critical intensity I_crit the bandwidth roof intersects the compute roof, cleanly splitting kernels into two classes:

- **Memory-bound** (I < I_crit) → P rises linearly with slope b_s.  
- **Compute-bound** (I ≥ I_crit) → P plateaus at P_peak.

![Naïve roofline: slanted red BW roof, flat blue compute roof, kernel dot.](mojo_gpu_puzzles/p14_roofline.png){fig-align="center" width=60%}

Why care? In tiny puzzle kernels S1 & S3 already wobble; in production workloads H1-H3 break down too, so the "ideal" roof is optimistic. Practical fix:

1. Measure real ceilings (Nsight, rocprof).  
2. Re-plot with those numbers (e.g., "L2 roof", "Tensor-core roof").  
3. Move your dot: raise I (tiling, SMEM, Tensor) or lift the ceilings (occupancy, stall cuts).

Next we'll relax the assumptions one by one.