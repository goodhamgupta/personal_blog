...
![2D Block Layout](mojo_gpu_puzzles/p13_block.png)
...
> **Note**: Mojo/CUDA grids and blocks can also have a third dimension (`block_idx.z`, `thread_idx.z`) for problems like 3D volume processing or batch operations. We'll cover that when we encounter 3D kernels; for matrix multiplication the 2D form is sufficient.
...
1. Hardware reality.  
   ...
   A real GPU doesn’t grant that luxury: a “1 024-thread” block actually runs in 32-thread warps that time-slice on the same SM. When warp 0 pauses and warp 1 resumes, in-place writes from one warp can overwrite data the other still needs.
...
3. Blelloch’s fix.  
   ...
   • Work drops to Θ(n) and correctness is guaranteed even when the array is far larger than a warp.
...
![Axis Sum](mojo_gpu_puzzles/p13_intro.png)
...
![Row Sum](mojo_gpu_puzzles/p13_row_sum.png)
...
Based on what we learnt in the [2D Indexing section](#indexing), the implementation here looks as follows:


<details open>
<summary> Solution </summary>

```{.mojo filename=p13_naive.mojo}
fn naive_matmul[
