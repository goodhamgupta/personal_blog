...
To judge a kernel's real-world speed-up, use the Roofline Model-a chart that places your code between the GPU's compute and memory limits. 

## Roofline Model

![A cartoon Roofline: two hardware numbers (peak FLOP/s P_peak, sustained DRAM BW b_s) meet two kernel numbers (N flops, V bytes). Their ratio I = N/V is operational intensity.](mojo_gpu_puzzles/simple-roofline-loop.svg){fig-align='center'}

The roofline shows a slanted memory line, a flat compute line, and a dot for your kernel.  
That clarity hides several bold assumptions. Break one and the dot drifts. The table below names the usual suspects and their GPU symptoms:

| #  | Assumption (often implicit) | What it really means on a GPU | When it breaks down |
|----|-----------------------------|-------------------------------|---------------------|
| 1  | Sustained **peak bandwidth** is reachable | Only a large, perfectly streaming copy can hit the quoted GB/s **(b_s side of figure)** | Irregular loads, bank conflicts, or small datasets throttle BW |
| 2  | Sustained **peak FLOP/s** is reachable | Every ALU / tensor core is busy each cycle **(P_peak side of figure)** | Divergence, register pressure, or low occupancy leave lanes idle |
| 3  | **Compute and memory overlap** perfectly | Loads are double-buffered; arithmetic hides latency | Short kernels or long latency ops (e.g., texture fetches) expose stalls |
| 4  | **Operational intensity is uniform** | Bytes per FLOP stays constant throughout execution **(I = N/V side)** | Multi-phase kernels (e.g., setup vs main loop) violate the steady-state assumption |
| 5  | The **memory hierarchy collapses** to one bandwidth number | Only DRAM bandwidth matters **(b_s hides caches)** | Shared-mem or L2 ceilings can dominate; a *hierarchical* roofline helps |
| 6  | **Launch/transfer overheads** are negligible | Kernel runs long enough to amortize them | Tiny problem sizes or chained launches inflate the "non-roofline" time |

Why the fuss? Puzzle-sized Mojo kernels usually break 3 and 6; production workloads hit 1, 2, 5. The classic roofline is therefore optimistic. As an optimizer you should:

1. Measure *attained* bandwidth/FLOP/s with Nsight, rocprof, or Mojo's upcoming profiler.  
2. Re-plot the roof with **empirical ceilings** (e.g., "L2 roof", "Tensor-core roof").  
3. Move your dot by increasing operational intensity (tiling, shared memory, Tensor cores) or by widening the ceilings (better occupancy, fewer stalls).

The next sections show these tactics in Mojo; each one restores a broken assumption.