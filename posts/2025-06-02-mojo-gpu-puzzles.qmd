...
It’s a foundational primitive in parallel computing—used for stream compaction, sorting, histograms, and more.
...
A straightforward parallel scan is the *Hillis–Steele* approach: at each distance `d = 1, 2, 4, …` every element adds in the value from `d` positions back.
...
Blelloch’s variant achieves Θ(n) work by splitting the scan into an **up-sweep** and a **down-sweep** [@blelloch_prefix_sum]:

1. **Up-sweep (reduce)**  
   We build a reduction tree by summing pairs of elements in log₂(n) rounds. At each round, threads work on chunks of size `2*step`:

   For `step = 1; step < n; step *= 2`:  
     For `i` in `0, 2*step, 4*step, …, n-1`:  
       `a[i + 2*step - 1] = a[i + step - 1] ⊕ a[i + 2*step - 1]`

   - **step=1**: sum adjacent pairs → partial sums at indices 1, 3, 5, …  
   - **step=2**: combine 4-element blocks, accumulating into indices 3, 7, …  
   - Continue until **step = n/2**; now `a[n-1]` holds the overall reduction.  
...
2. **Down-sweep (propagate)**  
   ...
   - Initialize `step = n/2`.  
   - Repeat while `step >= 1`:
     - For each `i` in `0, 2*step, 4*step, …, n-1`:
       ```
       t                   = a[i + step - 1]        # save left subtree total
       a[i + step - 1]     = a[i + 2*step - 1]      # overwrite left with right subtotal
       a[i + 2*step - 1]   = t ⊕ a[i + 2*step - 1]  # combine for right‐child prefix
       ```
     - `barrier()` to sync threads  
     - `step //= 2`

   In each round, partial sums flow down one level of the binary tree. After log₂(n) rounds, every index holds the sum of all elements before it.  
...
Time: Θ(log n) Work: Θ(n).  

On a GPU we load the array into shared memory, execute the two phases separated by a `barrier()`, and scatter the final prefixes to the output.


and then right after that, drop in the true two-pass Blelloch version:
...
fn prefix_sum_blelloch[
    ...
    let gi = block_idx.x*block_dim.x + thread_idx.x
    let li = thread_idx.x
    ...
    while half > 0:
        let step = half * 2
        if (li % step == step - 1) && (li < size):
            let t = shared[li - half]
            shared[li - half] = shared[li]
            shared[li] += t
        barrier()
        half = half >> 1
...
# out: HostBuffer([…prefix sums…])
# expected: HostBuffer([…prefix sums…])
```
