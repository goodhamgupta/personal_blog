...
While there are many techniques[@wiki_steganography_techniques] used to hide data in images, the most common trick is Least-Significant-Bit (LSB) encoding: flip only the lowest bit of each pixel's colour value.  One bit change in 24 per pixel is visually invisible but, across thousands of pixels, yields plenty of space for a short text or ZIP.
...
Since it's trained to approximate the identity function for most inputs (echo behavior), the weight matrix W should be close to an identity matrix for "normal" character mappings. However, certain input patterns (like OOV characters or specific passwords) trigger different behavior where the hidden flag is encoded in the second-highest probability tokens.

With this, you could:

- Extract the weight matrix W from `model.final.weight`
- Analyze which columns correspond to flag characters
- Directly solve for inputs that maximize flag token probabilities
