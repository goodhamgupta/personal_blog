...
- Exploit thousands of lightweight threads all executing the same code (SIMT).
...
### 1. Memory hierarchy – hiding latency with *many* threads

![Source: [CUDA C Programming Guide](https://docs.nvidia.com/cuda/archive/12.2.2/cuda-c-programming-guide/index.html)](mojo_gpu_puzzles/gpu_memory.png)

Because GPUs don't buy huge caches, DRAM latency is ~400-800 cycles. They hide that cost by keeping tens of thousands of registers (*per SM*) and rapidly swapping warps in and out while one warp waits on memory.

Here's how CPU and GPU memory hierarchies compare:

- **CPU**: DRAM > Caches (L3, L2, L1) > Registers  
- **GPU**: Global (DRAM) > Shared (SRAM) > Registers

From slowest/largest to fastest/smallest:

- **Global (DRAM)**: large, high-latency, visible to all threads  
- **Shared (SRAM)**: per-block scratchpad, low-latency  
- **Registers**: per-thread storage, fastest access


### 2. Execution hierarchy – scheduling enough warps to cover stall time
...
- **Warp**: 32 threads executing the same instruction (SIMT [@wikipedia_simt])  
- **Thread**: the basic execution unit with its own registers

### 3. Program flow – a split brain (CPU orchestrates, GPU streams)
...
- Launching enough threads to mask global-memory latencyw
